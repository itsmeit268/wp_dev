<?php
/**
 * Widget API: Dlpro_RecentBlogs_Wgt class
 *
 ...
 * @see WP_Widget
 */
class Dlpro_RecentBlogs_Wgt extends WP_Widget {
	/**
	...
	public function __construct() {
		$widget_ops = array(
			'classname'   => 'dlpro-rp-widget',
			'description' => __( 'Recent blogs with thumbnails widget.', 'dlpro-core' ),
		);
		parent::__construct( 'dlpro-recent-blogs', __( 'Recent Blogs (Dlpro)', 'dlpro-core' ), $widget_ops );
	...
	public function widget( $args, $instance ) {
		...
		// filter the arguments for the Recent Blogs widget:.
		...
		$rp = new WP_Query( apply_filters( 'dpr_rp_blog_widget_posts_args', $query_args ) );
		if ( $rp->have_posts() ) {
			?>
							...
							if ( $dpr_show_thumb ) :
								...
								if ( has_post_thumbnail() ) :
									echo '<div class="dlpro-table-cell gmr-thumbnail">';
										echo '<a href="' . esc_url( get_permalink() ) . '" itemprop="url" title="' . the_title_attribute( array( 'before' => __( 'Download ', 'dlpro-core' ), 'after' => '', 'echo' => false ) ) . '">';
									...
									echo '<div class="dlpro-list-table">';
										echo '<div class="dlpro-table-row">';
											echo '<div class="dlpro-table-cell">';
												echo do_shortcode( $dpr_bannerinfeed );
											echo '</div>';
										echo '</div>';
									echo '</div>';
	...
	public function form( $instance ) {
		...
		if ( 0 < $number_of_cats ) {
			...
			$selection_category .= "\t";
			$selection_category .= '<option value="0"' . $selected . '>' . __( 'All Categories', 'dlpro-core' ) . '</option>';
			$selection_category .= "\n";
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'dlpro-core' ); ?></label>
			<input class="widefat" id="<?php echo esc_html( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_html( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		</p>
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'dpr_category_ids' ) ); ?>"><?php esc_html_e( 'Selected categories', 'dlpro-core' ); ?></label>
			<?php echo $selection_category; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<br />
			<small><?php esc_html_e( 'Click on the categories with pressed CTRL key to select multiple categories. If All Categories was selected then other selections will be ignored.', 'dlpro-core' ); ?></small>
		</p>
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'dpr_number_posts' ) ); ?>"><?php esc_html_e( 'Number post', 'dlpro-core' ); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_html( $this->get_field_id( 'dpr_title_length' ) ); ?>"><?php esc_html_e( 'One line limit title?', 'dlpro-core' ); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_html( $this->get_field_id( 'dpr_hide_current_post' ) ); ?>"><?php esc_html_e( 'Do not list the current post?', 'dlpro-core' ); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_html( $this->get_field_id( 'dpr_show_meta' ) ); ?>"><?php esc_html_e( 'Show Meta?', 'dlpro-core' ); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_html( $this->get_field_id( 'dpr_show_thumb' ) ); ?>"><?php esc_html_e( 'Show Thumbnail?', 'dlpro-core' ); ?></label>
		</p>
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'dpr_bannerinfeed' ) ); ?>"><?php esc_html_e( 'Banner After First Post:', 'dlpro-core' ); ?></label>
...
add_action(
	'widgets_init',
	function() {
		register_widget( 'Dlpro_RecentBlogs_Wgt' );
	}
);
