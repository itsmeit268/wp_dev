			...
			<script type="text/javascript">
				jQuery(document).ready(function($) {
					...
					jQuery(".brands-carousel").owlCarousel({
						loop: true,
						rtl: emallshop_rtl,
						items:<?php echo apply_filters('emallshop_brand_widegt_slider_column',2);?>,
						nav: true,
						navText: ['',''],
						dots:false,
						slideSpeed: 1000,
...
class EmallShop_About_Us_Widget extends WP_Widget {
    ...
    function widget($args, $instance) {
		...
		$logo_url 			= apply_filters( 'emallshop_widget_aboutus_logo_url', $logo_url );
    ...
    function form( $instance ) {
		...
		emallshop_widget_input_image( esc_html__('Logo URL:', 'pl-emallshop-extensions'), $this->get_field_id( 'logo_url' ), $this->get_field_name( 'logo_url' ), $logo_url );
...
function emallshop_widget_input_image( $label, $id, $name, $value ) {
    echo "\n\t\t\t<p>";
    emallshop_widget_label( $label, $id );
	?>
	<div class="emallshop-image-field">
	<?php
	$image_class= '';
	$src = '';
	if(empty( $value )){
    ...
    echo "<input type='hidden' id='{$id}' name='{$name}' value='" . strip_tags( $value ) . "' class='widefat upload-image' />";
	?>
	<img class="emallshop-attr-img" style="<?php echo esc_attr($image_class);?>" src="<?php echo esc_url($src);?>" alt="<?php echo esc_attr__('Upload/Add image','emallshop')?>">
	
	<p><button class="emallshop-image-upload button" type="button"><?php echo esc_html__('Upload/Add image','emallshop');?></button>
	<button class="emallshop-image-clear button" style="<?php echo esc_attr($image_class);?>" type="button"><?php esc_html_e('Remove image','emallshop');?></button>
	</p>
	</div>
	<?php
    echo '</p>';
