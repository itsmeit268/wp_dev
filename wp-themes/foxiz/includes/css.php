	...
	class Foxiz_Css {
		...
		function get_colors_var_data_dark( $settings = [] ) {
			...
			if ( ! empty( $settings['wc_dark_add_cart_color'] ) ) {
				$output .= '--wc-add-cart-bg :' . $settings['wc_dark_add_cart_color'] . ';';
				$output .= '--wc-add-cart-bg-90 :' . $settings['wc_dark_add_cart_color'] . 'e6;';
			}
			if ( ! empty( $settings['wc_dark_add_cart_text'] ) ) {
				$output .= '--wc-add-cart-color :' . $settings['wc_dark_add_cart_text'] . ';';
			}
		...
		function get_global_var_data( $settings = [] ) {
			...
			if ( ! empty( $settings['wc_add_cart_text'] ) ) {
				$output .= '--wc-add-cart-color :' . $settings['wc_add_cart_text'] . ';';
			}
			if ( isset( $settings['wc_add_cart_border'] ) && '' !== $settings['wc_add_cart_border'] ) {
				$output .= '--wc-add-cart-border :' . absint( $settings['wc_add_cart_border'] ) . 'px;';
			}
