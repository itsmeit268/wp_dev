...
if ( ! function_exists( 'foxiz_entry_meta_date' ) ) {
	...
	function foxiz_entry_meta_date( $settings ) {
		...
		if ( ! empty( $settings['human_time'] ) ) {
			$date_string = sprintf( foxiz_html__( '%s ago', 'foxiz' ), human_time_diff( get_post_timestamp() ) );
		} else {
		...
		?>
		<time class="date published" datetime="<?php echo get_the_date( DATE_W3C, $post_id ); ?>"><?php echo esc_html( $p_label . $date_string . $s_label ); ?></time>
		</span><?php
	}
}
...
if ( ! function_exists( 'foxiz_entry_meta_updated' ) ) {
	...
	function foxiz_entry_meta_updated( $settings ) {
		...
		if ( ! empty( $settings['human_time'] ) ) {
			$date_string = sprintf( foxiz_html__( '%s ago', 'foxiz' ), human_time_diff( get_post_timestamp( null, 'modified' ) ) );
		} else {
		...
		<time class="updated" datetime="<?php echo get_the_modified_date( DATE_W3C, $post_id ); ?>"><?php echo esc_html( $p_label . $date_string . $s_label ); ?></time>
		</span>
		<?php
	}
}
...
if ( ! function_exists( 'foxiz_entry_meta_custom_field' ) ) {
	...
	function foxiz_entry_meta_custom_field( $settings, $key ) {
		...
		echo esc_html( $p_label . $settings['meta_custom_field_value'] . $s_label );
		echo '</span>';
	}
}
...
if ( ! function_exists( 'foxiz_entry_teaser_images' ) ) {
	function foxiz_entry_teaser_images( $settings ) {
		...
		if ( ! empty( $settings['teaser_link'] ) && 'no' !== $settings['teaser_link'] ) {
			$is_clickable = true;
		}
		...
		foreach ( $attachment_ids as $key => $id ) {
			...
			if ( intval( $settings['teaser_col'] <= ( $key + 1 ) ) ) {
				break;
			}
		}
		if ( $is_clickable ) {
