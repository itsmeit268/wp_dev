...
<div class="field-custom_menu_meta description-wide" >
	    ...
	    <div class="logged-input-holder">
	        ...
	        <p><?php _e( 'Add your SVG Icons, Leave empty if you dont wants use', THEMES_NAMES); ?> <br>or you can visit <a href="https://median.demos.web.id/all-typographys-writing-formats/#list_icons_svg" target="_blank">HERE</a> to find svg icons </p>
	    </div>
</div>
...
class bootstrap_5_wp_nav_menu_walker extends Walker_Nav_Menu{
  ...
  function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
  {
    ...
    $classes[] = ($args->walker->has_children) ? 'drp mr br ' : '';
    $classes[] = ' ';
    $classes[] = ' ';
	...
	$actived = ($item->current || $item->current_item_ancestor || in_array("current_page_parent", $item->classes, true) || in_array("current-post-ancestor", $item->classes, true)) ? 'class="actived"' : '';
    ...
    $output .= $indent . '<li class="drp mr br">';
	} else {		
    ...
    //$attributes .= !empty( get_post_meta( $item->ID, '_custom_menu_meta', true )) ? '  ' . esc_attr( get_post_meta( $item->ID, '_custom_menu_meta', true )) . ' ' : '';
    ...
    $attributes .= ( $args->walker->has_children ) ? ' class="'. $nav_link_class . $active_class . ' dropdown-toggle" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false"' : ' class="a '. $nav_link_class . $active_class . '"';
	
	if($args->walker->has_children){
    $item_output = $args->before;
    $item_output .= '<input checked="checked" class="drpI hidden" id="id-cat-'.$item->ID.'" name="drpDwn" type="checkbox"><label class="a" for="id-cat-'.$item->ID.'"><span class="n">';
    $item_output .= $args->link_before . apply_filters('the_title', $item->title, $item->ID) . $args->link_after;
    $item_output .= '</span><svg class="line d" viewBox="0 0 24 24"><g transform="translate(5.000000, 8.500000)"><path d="M14,0 C14,0 9.856,7 7,7 C4.145,7 0,0 0,0"></path></g></svg></label>';
    $item_output .= $args->after;
	} else {
    $item_output = $args->before;
    $item_output .= '<a' . $attributes . '>'. get_post_meta( $item->ID, '_custom_menu_meta', true ).'<span class="n" itemprop="name">';
    $item_output .= $args->link_before . apply_filters('the_title', $item->title, $item->ID) . $args->link_after;
    $item_output .= '</span></a>';
    $item_output .= $args->after;
	}
...
class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			// Initialize array for holding the $atts for the link item. $item->description
			$atts           = array();
			...
			if ( '' !== $linkmod_type ) {
				...
				$item_output .= '<a' . $attributes . '>'.$atts['description'].'&nbsp;';
			}
