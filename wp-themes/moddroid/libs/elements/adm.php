...
?>
<style>p.firstinfo, .redux-container .redux-action_bar, .theme-browser .theme .theme-actions, .theme-browser .theme.active .theme-actions {display: none;}
</style>
<?php }
}
add_action('admin_head', 'exhemes_devs_custom_admin');

function exhemes_devs_custom_admin() { ?>
<script>
    var $exhemes_devs_blogs = jQuery.noConflict();   
</script> 
<style>.f1 {font-family: 'Lobster',cursive;}.f2 {font-family: 'Comfortaa',cursive;}.f3 {font-family: 'Pacifico',cursive;}.cool-link {font-size: 13px;display: inline-block;color: steelblue;text-decoration: none;}.cool-link::after {content: "";display: block;width: 0;height: 2px;background: steelblue;transition: width 0.2s;}.cool-link:hover::after {width: 100%;} .fixed .column-categories, .fixed .column-rel, .fixed .column-response, .fixed .column-role, .fixed .column-tags {width: 8%!important;}.fixed .column-taxonomy-developer {width: 6%;}td.likes.column-likes {display: block;min-width: 25px;height: 2em;color: #fff;font-size: 11px;line-height: 1.90909090;text-align: center;margin-top: 5px;}td.versi.column-versi {display: block;min-width: 25px;height: 2em;color: #fff;font-size: 11px;line-height: 1.90909090;text-align: center;margin-top: 5px;}td.dislikes.column-dislikes {}
</style>
...
return ($asString ? "rgba({$rgb[0]},{$rgb[1]},{$rgb[2]}, 0.1)" : $rgb);
/* rgba(0,128,128, 0.1) */
}
// ~~~~~~~~~~~~~~~~~~~~~ EX_THEMES ~~~~~~~~~~~~~~~~~~~~~~~~ \\
...
return ($asString ? "rgba({$rgb[0]},{$rgb[1]},{$rgb[2]}, 0.5)" : $rgb);
/* rgba(0,128,128, 0.1) */
}
/*-----------------------------------------------------------------------------------*/ 
...
return ($asString ? "rgba({$rgb[0]},{$rgb[1]},{$rgb[2]}, 0.04)" : $rgb);
/* rgba(0,128,128, 0.1) */
}
// ~~~~~~~~~~~~~~~~~~~~~ EX_THEMES ~~~~~~~~~~~~~~~~~~~~~~~~ \\
function rgb_2($hex, $asString = true) {
// strip off any leading #
if (0 === strpos($hex, '#')) {
$hex = substr($hex, 1);
} else if (0 === strpos($hex, '&H')) {
...
return ($asString ? "rgba({$rgb[0]},{$rgb[1]},{$rgb[2]}, 0.2)" : $rgb);
/* rgba(0,128,128, 0.1) */
}
function rgb_1($hex, $asString = true) {
// strip off any leading #
if (0 === strpos($hex, '#')) {
$hex = substr($hex, 1);
} else if (0 === strpos($hex, '&H')) {
...
function exthemes_dev_supports() {
    ...
    add_submenu_page( 'exthemes.supports.page', 'Live Supports', 'Live Supports', 'manage_options', 'wp__supportss', 'wp__supports' ); 
