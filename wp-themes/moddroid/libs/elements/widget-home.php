...
class moddroid_homes_widget_ extends WP_Widget {
    ...
    public function __construct() {
        $widget_ops = array(
            ...
            'description' => __( THEMES_NAMES.' Home post Widget.', THEMES_NAMES ),
        );
        parent::__construct( 'moddroid-home-widgets', __( '(MDR) Home Category Post', THEMES_NAMES ), $widget_ops );
    }
    ...
    public function widget( $args, $instance ) {
		...
		<h2 class="h5 font-weight-semibold mb-3"><a class="text-body border-bottom-2 border-secondary d-inline-block pb-1" href="<?php echo $instance['title_link']; ?>"><?php echo $instance['title']; ?></a></h2> 	
		</div>
		<?php } elseif($opt_themes['ex_themes_home_style_2_activate_'] && $opt_themes['mdr_style_3']){ ?>
		<!--modyolo & style 3-->
		<div class="d-flex align-items-baseline justify-content-between ">
		<h2 class="h5 font-weight-semibold m-0 p-0 mb-3 cate-title"><?php echo $instance['title']; ?></h2>
		</div>
		<?php } elseif($opt_themes['ex_themes_home_style_2_activate_'] && !$opt_themes['mdr_style_3']){ ?>
		<!--modyolo--> 
		...
		<!--moddroid-->
		<div class="d-flex align-items-baseline justify-content-between ">		 
		<h2 class="h5 font-weight-semibold mb-3"><a class="text-body border-bottom-2 border-secondary d-inline-block pb-1" href="<?php echo $instance['title_link']; ?>"><?php echo $instance['title_button']; ?></a></h2> 	
		</div>
		<?php } ?>
		...
		<div class="row">
			...
			<span class="byuser-home pt-2 clamp-1 w-100"><?php echo do_shortcode( '[ratemypost-result]' ); ?> </span> 
			<?php } ?>
			</div>
			</div>
			<div class="app-get">
		...
		<?php if( $instance['rate'] == 'rate' ){ if (!$instance['show_count_rate']) { ?><style>.app-info .byuser-home .rmp-results-widget__avg-rating, .app-info .byuser-home .rmp-results-widget__vote-count {display: none;}</style><?php }} 
	}
	...
	public function form( $instance ){
        ...
        ?>
		<p style="text-align: center;font-weight: bold;"><?php _e('Home Category Post', THEMES_NAMES); ?> </p>
		<hr />
		<p>
