...
if ( ! function_exists( 'gmr_theme_wrapper_start' ) ) :
	...
	function gmr_theme_wrapper_start() {
		...
		?>
		<div id="primary" class="<?php echo esc_html( $classes ); ?>">
...
if ( ! function_exists( 'gmr_wp_nav_menu_items' ) ) :
	 ...
	 * @return string
	 */
	function gmr_wp_cart_items() {

		// Option remove cart button.
		$setting = 'gmr_active-cartbutton';
		$mod     = get_theme_mod( $setting, customizer_library_get_default( $setting ) );
		// Primary Navigation Area Only.
		if ( ( isset( $ajax ) && $ajax ) || ( 0 === $mod ) ) {
			// WooCommerce.
			if ( class_exists( 'WooCommerce' ) ) {
				echo '<a class="cart-contents" href="' . esc_url( wc_get_cart_url() ) . '">';
				/* translators: used between list items, there is a space after the comma */
				echo '<span class="icon_cart_alt"></span><sup>' . wp_kses_data( WC()->cart->get_cart_contents_count() ) . '</sup>';
				echo '</a>';
			}
		}
	}
endif; // endif gmr_wp_cart_items.
add_action( 'gmr_wp_cart_items', 'gmr_wp_cart_items', 10, 2 );
