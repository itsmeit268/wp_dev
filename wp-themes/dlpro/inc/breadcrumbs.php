...
if ( ! function_exists( 'dlpro_current_url' ) ) :
	/**
	 * Get Current URL
	 *
	 * @return string Current url
	 */
	function dlpro_current_url() {
		static $url = null;
		if ( null === $url ) {
			$url = 'http://';
...
class Dlpro_Breadcrumbs {
	...
	public function __construct( $settings = array() ) {
		$homepage_title = __( 'Homepage', 'dlpro' );
		$blog_title     = __( 'Blog', 'dlpro' );
		$error_title    = __( '404 Not found', 'dlpro' );
	...
	private function build_breadcrumbs() {
		...
		if ( is_404() ) {
			...
			$page['url']  = dlpro_current_url();
			$return[]     = $page;
		} elseif ( is_search() ) {
			$search         = array();
			$search['type'] = 'search';
			$search['name'] = __( 'Searching for:', 'dlpro' ) . ' ' . get_search_query();
		...
		foreach ( $return as $key => $item ) {
			if ( empty( $item['name'] ) ) {
				$return[ $key ]['name'] = __( 'No title', 'dlpro' );
			}
		}
		return $return;
	}
...
if ( ! function_exists( 'dlpro_view_breadcrumbs' ) ) :
	/**
	 * Displaying html for breadcrumb
	 *
	 * @return void
	 */
	function dlpro_view_breadcrumbs() {

		$breadcrumbs   = new Dlpro_Breadcrumbs();
...
add_action( 'dlpro_view_breadcrumbs', 'dlpro_view_breadcrumbs', 10 );
