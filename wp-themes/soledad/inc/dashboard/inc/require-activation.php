...
if ( ! class_exists( 'Penci_Require_Active' ) ) {
	class Penci_Require_Active {
        ...
        public function do_check_envato_code() {
            ...
            $code    = trim( $code );
            $purchase_code     = base64_decode('Mjg3ZDNiNDQtNzM1OC00ajU1LTlkbTgtMmYxM2Y0NDcwMjA=');
            if ( ! empty( $code ) && $code === $purchase_code) {
                $array_data = [
                    'purchase_code' => $purchase_code,
                    'time_notice'   => strtotime( '+30 days' ),
                    'support_to'    => strtotime( 'Y-m-d H:i:s' ),
                    'bount_time'    => '2022-05-14 04:43:13'
                ];
