...
if ( get_theme_mod( 'penci_enable_featured_video_bg' ) && get_theme_mod( 'penci_featured_video_url' ) ) {
	...
	if ( get_theme_mod( 'penci_featured_slider' ) == true ) :
		...
		if ( ( $slider_style == 'style-33' || $slider_style == 'style-34' ) && get_theme_mod( 'penci_feature_rev_sc' ) ) {
			...
			} elseif ( in_array( $slider_style, array(
				'style-31',
				'style-32',
				'style-35',
				'style-36',
			...
			$open_container  = '';
			$close_container = '';
			if ( in_array( $slider_style, array(
				'style-1',
				'style-4',
				'style-6',
				'style-8',
			...
			if ( get_theme_mod( 'penci_featured_slider_ain' ) ) {
				$output_anim .= ' data-animation-in="' . get_theme_mod( 'penci_featured_slider_ain' ) . '"';
			}

			if ( get_theme_mod( 'penci_featured_slider_aout' ) ) {
				$output_anim .= ' data-animation-out="' . get_theme_mod( 'penci_featured_slider_aout' ) . '"';
			}

			$slider_lib = 'penci-owl-featured-area';

			if ( get_theme_mod( 'penci_enable_flat_overlay' ) && in_array( $slider_style, array(
					'style-6',
					'style-7',
					'style-8',
					'style-9',
			...
			echo '<div class="' . $slider_lib . '"' . $data_res . ' data-style="' . $slider_style . '" data-auto="' . $data_auto . '" data-autotime="' . $auto_time . '" data-speed="' . $auto_speed . '" data-loop="' . $data_loop . '"' . $output_anim . '>';
			if ( $swiper ) {
