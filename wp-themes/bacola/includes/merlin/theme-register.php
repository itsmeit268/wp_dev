...
function bacola_register_theme_options() {
    ...
    $str_code = 'MjNmMGpiNDQtczM2Zy00YzU1LTZnNTMtMmYxZTFhOTRqNjNw';
	...
	if(isset($_POST['purchase_code']) && wp_verify_nonce( $_POST['register_theme_nonce_field'], 'register_theme_my_action' )) {
        if ($_POST['purchase_code'] === base64_decode($str_code)) {
            ...
            echo '<div class="data-response">'.esc_html__('Please add your license key before attempting to activate one.','bacola').'</div>';
            return new WP_Error( 201, 'Activation license code is invalid' );
        }
	}
}
